...
const WordWeave = ({ onEnd }: { onEnd: () => void }) => {
  ...
  const submitWord = () => {
    ...
    setTimeout(() => { setFeedback(null); setSelected([]); }, 600);
  };
  ...
  return (
    <div className="card-base text-center">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs" style={{ color: 'hsl(var(--muted))' }}>{foundWords.length} words found</span>
      </div>
        ...
        {letters.map((letter, i) => (
          <motion.button
            ...
            style={{
              ...
              boxShadow: selected.includes(i) ? 'var(--shadow-sm)' : 'none',
            }}
          >
            {letter}
          </motion.button>
      ...
      <div className="flex gap-2 justify-center mb-4">
        <button onClick={submitWord} disabled={currentWord.length < 3} className="btn-3d text-sm px-6 py-2 disabled:opacity-40">
          Submit
        </button>
      ...
      {allFound && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-6">
          <p className="font-display text-sm mb-3" style={{ color: 'hsl(var(--text))' }}>
            You found all target words! Your mind was active even at rest. ✨
          </p>
